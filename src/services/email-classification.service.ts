...
export class EmailClassificationService {
  private readonly HUGGING_FACE_API_URL: string =
    process.env.HUGGING_FACE_API_URL || "https://api-inference.huggingface.co/models/facebook/bart-large-mnli";
  ...
  async classifyEmail(emailContent: string, candidateLabels: string[]): Promise<ClassificationResultDto[]> {
    try {
      ...
      const results: ClassificationResultDto[] = labels.map((label: string, index: number) => ({
        label,
        score: scores[index]
      }));

      // Find the result with the highest score
      const highestScoreResult = results.reduce((max, current) => (current.score > max.score ? current : max), results[0]);
